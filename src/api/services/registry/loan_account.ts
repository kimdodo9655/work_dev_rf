...
export const registryLoanAccountAPI = {
  async getDetail(params: GetLoanPaymentAccountsParams) {
    ...
    // --------------------------------------------------
    return apiHelpers.get(API.REGISTRY_LOAN_ACCOUNT.DETAIL(params.registryManagementNumber))
  },
  ...
  async getList(params: GetLoanPaymentAccountListParams) {
    ...
    // --------------------------------------------------
    return apiHelpers.get(API.REGISTRY_LOAN_ACCOUNT.LIST(params.registryManagementNumber))
  },

  async save(params: SaveLoanPaymentAccountsParams) {
    // --------------------------------------------------
    // [R02R-03][PUT - /api/registry/progress/{registryManagementNumber}/loan-payment-accounts] 대출금 지급계좌 일괄 저장
    // --------------------------------------------------
    return apiHelpers.put(API.REGISTRY_LOAN_ACCOUNT.SAVE(params.registryManagementNumber))
  },

  async upload(params: UploadReceiptParams) {
    // --------------------------------------------------
    ...
    return apiHelpers.post(
      API.REGISTRY_LOAN_ACCOUNT.UPLOAD(params.registryManagementNumber, params.loanPaymentAccountId)
    )
  },
